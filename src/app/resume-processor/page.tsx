...
export default function ResumeProcessorPage() {
  ...
  const addCustomKeyword = () => {
    if (customKeyword.trim() && !processingOptions.customKeywords.includes(customKeyword.trim())) {
                  ...
                  <CardContent>
                    ...
                    <Accordion sx={{ mt: 2 }}>
                      <AccordionSummary expandIcon={<ExpandMoreIcon />}>
                        <Typography variant="body2">Processing Steps</Typography>
                      </AccordionSummary>
                      <AccordionDetails>
                        <List dense>
                          {job.progress.steps.map((step, index) => (
                            <ListItem key={index}>
                              <ListItemIcon>
                                ...
                                {step.status === 'processing' && <ScheduleIcon color="primary" fontSize="small" />}
                                {step.status === 'failed' && <ErrorIcon color="error" fontSize="small" />}
                                {step.status === 'pending' && <ScheduleIcon color="disabled" fontSize="small" />}
                              </ListItemIcon>
                              <ListItemText
                                primary={step.name}
                ...
                <Grid item xs={12}>
                  <FormControl fullWidth>
                    ...
                    <Select
                      ...
                      onChange={(e) => setProcessingOptions(prev => ({
                        ...prev,
                        outputFormat: e.target.value as 'docx' | 'pdf' | 'html'
                      }))}
                    >
              ...
              {selectedResumes.length > 0 && (
                <Alert severity="info" sx={{ mt: 3 }}>
                  <Typography variant="body2">
                    Processing {selectedResumes.length} resume{selectedResumes.length > 1 ? 's' : ''} with selected options.
                  </Typography>
                </Alert>
              )}
            </Paper>
